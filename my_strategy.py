...
class MyStrategy:
    ...
    def __init__(self, constants: Constants):
        ...
        for obs in constants.obstacles:
            MyStrategy.ObstacleCharges[obs.id] = ObstacleCharge(Vec(obs.position.x, obs.position.y), -10_000,
                                                                Float(obs.radius + 1.5))

    def init(self, game: Game):
        self.zone = ZoneCharge(
            Vec(game.zone.next_center.x, game.zone.next_center.y), 1_000,
            Circle(Vec(game.zone.current_center.x, game.zone.current_center.y), Float(game.zone.current_radius - 1)),
            Circle(Vec(game.zone.next_center.x, game.zone.next_center.y), Float(game.zone.next_radius - 1)))
        self.my_units = []
        self.near_obstacles = {}
        for unit in game.units:
            if unit.player_id == game.my_id:
                ...
                for id_obs, obs in MyStrategy.ObstacleCharges.items():
                    obs: Charge
                    if (obs.centre_pos - unit.position).len <= 15:
    ...
    def get_order(self, game: Game, debug_interface: Optional[DebugInterface]) -> Order:
        ...
        for unit in self.my_units:
            ...
            if count_charge(self.near_obstacles[unit.id], self.zone, max_simulated_pos) > now_charge:
